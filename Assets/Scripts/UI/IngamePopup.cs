...
public class IngamePopup : MonoBehaviour {
	...
	[HideInInspector]
	public bool isFlagsAndGenerals;
	public void AgreeTerms() {
		MyPlayerPrefs.instance.SetInt("agreeToPolicy", 1);
		MyPlayerPrefs.instance.SaveData();

		if (isFlagsAndGenerals) {
			FlagsAndGeneralsPopup p = Instantiate(menuMasta.flagsAndGeneralsPrefab,
				GameObject.Find("Canvas").transform).GetComponent<FlagsAndGeneralsPopup>();
			p.manager = menuMasta;
		} else {
	...
	IEnumerator RedeemCode() {
		//note: replace with main.indiewargames.net (set up centralized redeem code and recent messages)
		UnityWebRequest r = UnityWebRequest.Get(CustomFunctions.ConvertToUtf8(
			"http://main.indiewargames.net:8001/send_feedback?text=" + couponCode.text));
	...
	IEnumerator CensorMap(int value) {
		UnityWebRequest r = UnityWebRequest.Get(CustomFunctions.ConvertToUtf8(CustomFunctions.getURL() + "mark_map?author=" + mapAuthor + "&map_name=" + mapName + "&password=ArmchairCommander_Admin" + "&mark_value=" + value.ToString()));
	...
	IEnumerator AddFeatured(bool isAdding) {
		if (isAdding) {
			UnityWebRequest r = UnityWebRequest.Get(CustomFunctions.ConvertToUtf8(CustomFunctions.getURL() + "add_one_featured?map_name=" + mapName + "&author=" + mapAuthor + "&password=retrocombat_admin"));

			alreadyRequesting = true;
			yield return r.SendWebRequest();
		} else {
			UnityWebRequest r = UnityWebRequest.Get(CustomFunctions.ConvertToUtf8(CustomFunctions.getURL() + "remove_featured?name=" + mapName + "&author=" + mapAuthor + "&password=retrocombat_admin"));
	...
	IEnumerator downloadMapJson() {
		...
		try {
			//print(jsonData);
			m = JsonUtility.FromJson<MapInfo>(jsonData);
		...
		//download necessary flags TODO: download generals
		List<string> requiredFlags = new();
		List<string> downloadedFlags = new(PlayerData.instance.playerData.flags.Keys);
		List<string> requiredGenerals = new();
		List<string> downloadedGenerals = new(PlayerData.instance.playerData.customGenerals.Keys);

		List<string> standardGenerals = new(PlayerData.instance.generals.Keys);

		foreach (CountryData c in m.countryDataValues) {
			if (c.aiGenerals == null) continue;
			foreach (string s in c.aiGenerals.Keys) {
				if (!standardGenerals.Contains(s) && !downloadedGenerals.Contains(s) &&
					!requiredGenerals.Contains(s) && s != "") {
					print($"Need to download general: {s}");
					requiredGenerals.Add(s);
				}
			}
		}
		foreach (string s in m.soldierGenerals) {
			if (!standardGenerals.Contains(s) && !downloadedGenerals.Contains(s) &&
				!requiredGenerals.Contains(s) && s != "") {
				print($"Need to download general: {s}");
				requiredGenerals.Add(s);
		...
		foreach (string s in m.countryCustomFlagOverridesValues) {
			if (!MenuManager.instance.customFlagNames.Contains(s) && s != "") {
				if (downloadedFlags.Contains(s)) {
					print($"Already downloaded flag: {s}");
				} else {
					print($"Need to download flag: {s}");
					requiredFlags.Add(s);
				}
			}
		}
		for (int index = 0; index < requiredFlags.Count; index++) {
			title.text = CustomFunctions.TranslateText("Downloading Flags") + $" ({index + 1}/{requiredFlags.Count})";
			yield return GetFlagCoroutine(requiredFlags[index]);
		}
		for (int index = 0; index < requiredGenerals.Count; index++) {
			title.text = CustomFunctions.TranslateText("Downloading Generals") + $" ({index + 1}/{requiredGenerals.Count})";
			yield return GetGeneralCoroutine(requiredGenerals[index]);
		}
		SceneManager.LoadScene(1);
	}
	private IEnumerator GetGeneralCoroutine(string generalName) {
		string serverURL = "http://main.indiewargames.net:8081/";
		using UnityWebRequest www = UnityWebRequest.Get(serverURL + $"get_general_data?general_name={generalName}");
		yield return www.SendWebRequest();

		General g = null;
		if (www.result == UnityWebRequest.Result.Success) {
			if (www.downloadHandler.text.Contains("not found")) {
				print($"general not found: {generalName}");
				yield break;
			}
			...
			//for some reason, when parsing like this Unity gets rid of the \" that gets
			//added to every quotation mark
			List<BrowseGeneralPopup.GeneralData> generalDataList = JsonUtility.FromJson<
				BrowseGeneralPopup.GeneralDataList>(jsonResponse).generals;

			g = JsonUtility.FromJson<General>(generalDataList[0].general_json);
		} else {
			Debug.LogError("Data load request failed, error: " + www.error);
			yield break;
		}

		//load photo
		using UnityWebRequest www2 = UnityWebRequestTexture.GetTexture(
		...
		if (www2.result == UnityWebRequest.Result.Success) {
			...
			print(g);
			g.photo = texture.EncodeToPNG();
		} else {
			Debug.LogError("Image load request failed, error: " + www2.error);
			yield break;
		}

		//save general after photo is loaded as well
	...
	private IEnumerator GetFlagCoroutine(string flagName) {
		string serverURL = "http://main.indiewargames.net:8081/";
		...
		if (www.result == UnityWebRequest.Result.Success) {
			...
			if (!PlayerData.instance.playerData.flags.ContainsKey(flagName)) {
				PlayerData.instance.playerData.flags.Add(flagName, texture.EncodeToJPG());
				PlayerData.instance.saveFile();
			}
		} else {
			Debug.LogError("Image load request failed, error: " + www.error);
		}
		yield return null;
	}
